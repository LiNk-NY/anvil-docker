...
def get_r_doc(params):
  ...
  # Memory-efficient R script that reads one DESCRIPTION file at a time
  # instead of calling installed.packages() which loads all fields for all
  # packages into memory at once and causes OOM kills on large Bioconductor
  # images with many packages installed.
  r_script = """libs <- .libPaths()
rows <- list()
for (lib in libs) {
  pkgdirs <- list.dirs(lib, full.names = FALSE, recursive = FALSE)
  for (p in pkgdirs[nchar(pkgdirs) > 0]) {
    f <- file.path(lib, p, "DESCRIPTION")
    if (file.exists(f)) {
      d <- tryCatch(
        read.dcf(f, fields = c("Package", "Version", "Priority")),
        error = function(e) NULL
      )
      if (!is.null(d) && is.na(d[1, "Priority"]))
        rows[[length(rows) + 1]] <- data.frame(
          Package = as.character(d[1, "Package"]),
          Version = as.character(d[1, "Version"]),
...
if (length(rows) > 0) {
  ip <- do.call(rbind, rows)
  print(ip, row.names = FALSE)
}
"""

  with tempfile.NamedTemporaryFile(mode='w', suffix='.r', delete=False) as tmpf:
    tmpf.write(r_script)
    tmpfile = tmpf.name

  try:
    utils.shell_exec("docker cp {} {}:/tmp/pkg_list.r".format(tmpfile, image_dir))
    raw_r_doc = utils.docker_exec(image_dir, "Rscript /tmp/pkg_list.r")
    utils.docker_exec(image_dir, "rm -f /tmp/pkg_list.r")
  finally:
    os.unlink(tmpfile)
